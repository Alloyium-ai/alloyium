...
const A2A_AGENT_ID_RE = /^[a-z0-9-]{1,64}$/
...
export function isCodexWriteRequester(agentId: string): boolean {
  const id = agentId.trim().toLowerCase()
  return A2A_AGENT_ID_RE.test(id) && !isCodexRealtimeSessionPeer(id)
}
