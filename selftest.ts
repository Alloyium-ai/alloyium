...
// a temporary self-owned JetStream stream, and benign claude.channels.selftest*
...
const TEST_KEY = 'claude-channels:selftest'
const enc = (o: unknown) => new TextEncoder().encode(JSON.stringify(o))

// Point the bridge at our throwaway key BEFORE importing it (env read at import).
process.env.SUBS_KEY = TEST_KEY
process.env.CONTROL_SUBJECT = 'claude.channels.selftest.control'

const specs = [
  { subject: 'claude.channels.selftest', mode: 'core' },
  { subject: 'claude.channels.selftest.js', mode: 'jetstream', stream: 'CLAUDE_SELFTEST', durable: 'claude-selftest-consumer', filter_subject: 'claude.channels.selftest.js' },
]
...
try {
  ...
  nc = await connect({ servers: NATS_URL, name: 'claude-channels-selftest' })
  const jsm = await nc.jetstreamManager()
  try { await jsm.streams.add({ name: 'CLAUDE_SELFTEST', subjects: ['claude.channels.selftest.js'] }) } catch {}
  ...
  nc.publish('claude.channels.selftest', enc({ kind: 'CORE_TEST', hello: 'core-nats' }))
  await nc.jetstream().publish('claude.channels.selftest.js', enc({ kind: 'JS_TEST', hello: 'jetstream' }))
  await Bun.sleep(1200)

  const gotCore = received.some((r) => r.subject === 'claude.channels.selftest' && r.mode === 'core')
  const gotJs = received.some((r) => r.subject === 'claude.channels.selftest.js' && r.mode === 'jetstream')
